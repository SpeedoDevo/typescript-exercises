...
type OldApiCall<T> = (cb: (res: ApiResponse<T>) => void) => void;
function promisify<F extends OldApiCall<any>>(
  arg: F
): () => Promise<F extends OldApiCall<infer T> ? T : never> {
  return () =>
    new Promise((resolve, reject) => {
      arg((res) =>
        res.status === "success" ? resolve(res.data) : reject(res.error)
      );
    });
}
